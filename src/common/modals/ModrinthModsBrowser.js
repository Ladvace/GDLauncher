...
const ModsListWrapper = ({
  ...
  const Row = memo(({ index, style, data }) => {
    ...
    const isInstalled = installedMods.find(
      v =>
        (v.projectID && v.projectID === item?.id) ||
        v?.id === item?.mod_id.split('-')[1]
    );

    const id = item?.mod_id.split('-')[1];
    ...
    return (
      ...
      >
        ...
        <RowInnerContainer>
          <RowContainerImg img={item.icon_url} />
          <div
            css={`
...
const ModsBrowser = ({ instanceName, gameVersion }) => {
  ...
  const [filterType, setFilterType] = useState('downloads');
  const [loaderType, setLoaderType] = useState(instance.modloader.loaderType);
  const [searchQuery, setSearchQuery] = useState('');
  const [hasNextPage, setHasNextPage] = useState(false);
  const [nextPage, setNextPage] = useState(itemsNumber);
  ...
  }, [filterType, loaderType]);

  useEffect(() => {
    loadMoreMods();
  }, []);

  const loadMoreMods = async (reset = false) => {
    ...
    const newMods = reset ? data.hits : mods.concat(data.hits);
    ...
    if (lastRequest === reqObj) {
      ...
      setMods(newMods);
      setHasNextPage(
        newMods.length % itemsNumber === 0 && newMods.length !== 0
      );

      setNextPage(nextPage + itemsNumber);
    }
  };
      ...
      <Container>
        <Header>
          ...
          >
            ...
            <Select.Option value="downloads">Popularity</Select.Option>
            <Select.Option value="updated">Last Updated</Select.Option>
            <Select.Option value="newest">Last Created</Select.Option>
          </Select>
          <Select
            css={`
              width: 160px;
